...
- [x] **Agent interface contract** — `AgentSpec` document: tool schema, approval hook signature, context-bus message format, error semantics *(promoted: every later phase depends on it)* → [docs/AGENT_SPEC.md](https://docs.01p5.com/reference/agent-spec) (draft v0.1)
- [x] Proof of concept: single agent (Sysadmin — read-only `kubectl` / log queries) executing a task end-to-end — live since W4 against the PVE cluster, see [docs/LIVE_DEMO.md](https://docs.01p5.com/guide/live-deployment)
- [x] Docker Compose dev environment — `docker-compose.yml` at repo root; production deploy uses the same multi-stage Dockerfile
- [x] Decide two open questions that shape the bus: orchestrator-only delegation in v1; long-running ops stream `progress` messages (see [AGENT_SPEC.md "Locked decisions"](https://docs.01p5.com/reference/agent-spec))
...
- [x] Harden context bus — `RedisStreamsBus` implements the same `Bus` Protocol as `InMemoryBus`; orchestrator now bus-agnostic with per-task wait events. Decision in [docs/BUS_DECISION.md](https://docs.01p5.com/reference/design-notes)
...
- **Group chat / sub-agents** ([docs/SUBAGENTS_PLAN.md](https://docs.01p5.com/reference/design-notes)) — the dashboard chat became a group-chat *ticket*: a non-routable `main` coordinator `dispatch`es subtasks to specialists and `ask_agent`s them direct questions; per-(ticket,agent) checkpoints; ticket lifecycle (close → summarize-to-memory → discard). Phases 1–3 shipped; Phase 4 (MCP push-notifications) deferred.
...
- [x] **(decided W2, revisited post-W10)** Agent-to-agent delegation: orchestrator-only in v1 — **revisited.** The dashboard now runs a group-chat model where a `main` coordinator `dispatch`es to specialists and agents `ask_agent` each other (mediated by the orchestrator, logged to the ticket transcript). The CLI keeps the router-picks-one model. See [docs/SUBAGENTS_PLAN.md](https://docs.01p5.com/reference/design-notes).
- [x] **(decided W2)** Long-running ops (Terraform apply): sync-with-progress-events. No async job model in v1.
- [x] **(decided W5)** Message queue for context bus: **Redis Streams** for v2; in-memory bus stays for tests/single-process dev. See [docs/BUS_DECISION.md](https://docs.01p5.com/reference/design-notes).
