"""Minimal CLI for the HPC agent — same shape as the other agents'.

Note: without gpu-mcp + slurm-mcp wired the agent has no tools and
will produce a structured response saying so. The CLI is mostly
useful in dev with the MCP servers pre-attached via Olympus's
build_default_server(mcp_servers=...).
"""
from __future__ import annotations

import argparse
import json
import os
import sys
import uuid
from dataclasses import asdict

from agentlib import AgentContext, ConsoleApprovalHook, JsonlAuditLogger, TaskMessage

from .agent import HPCAgent


def main() -> int:
    parser = argparse.ArgumentParser(prog="olympus-hpc")
    parser.add_argument("request", help="Natural-language task for the agent")
    parser.add_argument(
        "--audit-log",
        default=os.path.expanduser("~/.olympus/audit.jsonl"),
    )
    args = parser.parse_args()

    ctx = AgentContext(
        approval=ConsoleApprovalHook(),
        audit=JsonlAuditLogger(args.audit_log),
    )
    task = TaskMessage(task_id=str(uuid.uuid4()), natural_language=args.request)

    agent = HPCAgent()
    result = agent.handle(task, ctx)
    print(json.dumps(asdict(result), indent=2, default=str))
    return 0 if result.status == "success" else 1


if __name__ == "__main__":
    sys.exit(main())
